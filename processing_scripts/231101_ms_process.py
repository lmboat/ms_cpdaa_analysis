# Created Date: 201116
# Modified Date: 231101
# Author: Lisa Boatner
# Technical Updates: Update for keeping unmodified and modified peptides in the output. 
# Also updated uniprot_dict to skip unmatched proteins & skip rows with no identifiers.
...
class Game:
  ...
  def get_identifiers(self, line, header, file, experiment_id, experiments, col_name):
    ...
    if line[header.index("Identifier")] not in self.identifiers_names:
      ...
      identifier = Identifier(line[header.index("Protein ID")], line[header.index("Protein Description")], line[header.index("Identifier")], line[header.index("Gene")], line[header.index("Peptide")], correlation, line[header.index("Spectral Count")], line[header.index("Modifications")], line[header.index("Amino Acids")], line[header.index("Modification Masses")], line[header.index(col_name)], file, self.result_files, self.args, experiment_id, experiments)
    ...
    # peptide, ratio, dataset
      ...
      current.update(line[header.index("Peptide")], correlation, line[header.index("Spectral Count")], line[header.index("Modifications")], line[header.index("Amino Acids")], line[header.index("Modification Masses")], line[header.index(col_name)], file, experiment_id)
  ...
  def run_rep(self):
    ...
    if self.experiment_db == "msf":
      ...
      # read peptide_label_quant versions for isotop experiments
        ...
        if (self.args.database_version == '20'):
...
class Identifier(Protein):
  def __init__ (self, protein, description, identifier, name, peptide, correlation, spectral_count, modifications, amino_acids, masses, ratio, dataset, pep_files, args, experiment_id, experiments):
    self.args = args
    ...
    for i in range(len(pep_files)):
      ...
      self.ratio_dict[pep_files[i]] = []
    self.ratio_mean_dict, self.ratio_median_dict, self.ratio_stdev_dict = self.ratio_dict.copy(), self.ratio_dict.copy(), self.ratio_dict.copy()
    self.update(peptide, correlation, spectral_count, modifications, amino_acids, masses, ratio, dataset, experiment_id)

  def update(self, peptide, correlation, spectral_count, modifications, amino_acids, masses, ratio, dataset, experiment_id):
    self.peptides = self.update_list(peptide, self.peptides)
    self.peptide_count = len(self.peptides)
...
class Peptide(Protein):
  def __init__ (self, peptide, charge_state, prob, correlation, localization, protein, identifier, description, spectral_count, intensity, miscleavages, modifications, amino_acids, masses, ratio, dataset, pep_files, args, experiment_id, experiments):
    self.args = args
    ...
    for i in range(len(pep_files)):
      ...
      self.ratio_dict[pep_files[i]] = []
    self.ratio_mean_dict, self.ratio_median_dict, self.ratio_stdev_dict = self.ratio_dict.copy(), self.ratio_dict.copy(), self.ratio_dict.copy()
    self.update(charge_state, prob, correlation, localization, protein, identifier, description, spectral_count, intensity, modifications, amino_acids, masses, ratio, dataset, experiment_id)

  def update(self, charge_state, prob, correlation, localization, protein, identifier, description, spectral_count, intensity, modifications, amino_acids, masses, ratio, dataset, experiment_id):
